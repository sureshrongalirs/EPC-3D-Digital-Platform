...
// The reload-race guard that used to live here (applyGeorefRotationIfStillCurrent) was
// replaced by internal/reloadGuard.ts's runIfStillCurrent -- see its own doc comment and
// reloadGuard.test.ts for why (guarding just the rotation call left the identical crash one
// line later, in code the narrower wrapper never covered).
